...
// Re-export ConcurrencyError from the main delta module
export { ConcurrencyError } from './index.js'
export type { ConcurrencyErrorOptions } from './index.js'

// Import error utilities from errors module
import { hasRetryableProperty, isRetryableError } from '../errors.js'

// Re-export isRetryableError for convenience
...
export interface RetryConfig {
  ...
  readonly onRetry?: (info: RetryInfo) => boolean | void
  /** Called on successful completion */
  readonly onSuccess?: (info: SuccessInfo) => void
...
/**
 * Internal configuration that extends RetryConfig with testing utilities.
...
/**
 * Interface for errors that have metrics attached.
 */
export interface ErrorWithMetrics extends Error {
  metrics: RetryMetrics
}
...
export async function withRetry<T>(
  ...
  // Cast to internal config to access testing utilities
  const internalConfig = config as InternalRetryConfig
  // Merge with defaults
  ...
  const delayFn = internalConfig._delayFn ?? defaultDelay
  ...
  // Helper to build failure metrics and throw
  const throwWithMetrics = (err: Error) => {
    ...
    if (returnMetrics) {
      ;(err as ErrorWithMetrics).metrics = metrics
    }
  ...
  while (true) {
    ...
    try {
      ...
      // Check if previous onRetry returned false (abort after this attempt failed)
      if (onRetryAborted) {
        throwWithMetrics(err)
      }
      ...
      if (config.onRetry) {
        ...
        // If onRetry explicitly returns false, mark for abort on next iteration
        // This allows the current delay to complete but prevents the next fn() call
