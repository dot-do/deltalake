[**@dotdo/deltalake v0.0.1**](../README.md)

***

[@dotdo/deltalake](../modules.md) / errors

# errors

## Classes

- [DeltaLakeError](classes/DeltaLakeError.md)
- [StorageError](classes/StorageError.md)
- [FileNotFoundError](classes/FileNotFoundError.md)
- [VersionMismatchError](classes/VersionMismatchError.md)
- [S3Error](classes/S3Error.md)
- [ConcurrencyError](classes/ConcurrencyError.md)
- [CDCError](classes/CDCError.md)
- [ValidationError](classes/ValidationError.md)

## Interfaces

- [ConcurrencyErrorOptions](interfaces/ConcurrencyErrorOptions.md)

## Type Aliases

- [CDCErrorCode](type-aliases/CDCErrorCode.md)

## Functions

- [isDeltaLakeError](functions/isDeltaLakeError.md)
- [isStorageError](functions/isStorageError.md)
- [isConcurrencyError](functions/isConcurrencyError.md)
- [isCDCError](functions/isCDCError.md)
- [isValidationError](functions/isValidationError.md)
- [isRetryableError](functions/isRetryableError.md)
- [isFileNotFoundError](functions/isFileNotFoundError.md)
- [isVersionMismatchError](functions/isVersionMismatchError.md)
